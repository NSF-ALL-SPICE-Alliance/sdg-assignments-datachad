...
```
```{r}
importance_dfi <- as.data.frame(rf_cleanfuel$importance)
```
```{r}
...
ggplot(importance_dfi_top_10, aes(x = `%IncMSE`, y = reorder(variable, `%IncMSE`))) +
  geom_bar(stat = "identity", fill = "steelblue", color = "black") +
  theme_minimal() +
  labs(title = "Most Important Variables for Clean Fuel Consumption",
       subtitle = "Top 10",
       y = "SDG Indicator",
       x = "Feature Importance (% Increase in Mean Squared Error)")
```
```{r}
pd <- pdp::partial(rf_cleanfuel, pred.var = c("normalized_score_sdg3_pollmort", "normalized_score_sdg6_freshwat"))
# Default PDP
plotPartial(pd)
```
...
```{r}
pdp::partial(rf_cleanfuel, pred.var = "normalized_score_sdg3_pollmort", plot = TRUE)
```
