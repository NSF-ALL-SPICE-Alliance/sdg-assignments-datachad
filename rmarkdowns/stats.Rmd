...
```{r, warning=FALSE, message=FALSE}
sdg_goal_chads_one <- ggplot(sdr_data, aes(x = goal_1_score, 
                     y = goal_3_score, label = country)) +
  geom_point() +
  geom_smooth() +
...
```
There is a very high correlation between the goal scores for "No Poverty" and "Health and well being".  It makes sense because where there is less poverty, in general the well being of that demographic has nourishment and clean water more readily available.  Medicine and health care is also a strong point for demographics with less poverty so individuals well being commonly rates higher.
