...
```{r}
goal_da_score <- ggplot(sdr_data, aes(x = goal_4_score, y = goal_7_score, color =regions_used_for_the_sdr)) +
  geom_point() +
  theme_dark() +
  scale_fill_viridis_d(option = "magma") +
  labs(title = "Data Braddahz",
       x = "Score 4 SDG",
       y = "All Dem Countries",
       color = "Da Spotty")
...
```{r}
plot_scores_plot <- ggplot(data = sdr_data, aes(x= goal_3_score, y = goal_4_score, color = regions_used_for_the_sdr, label = country)) +
  geom_point() +
  theme_minimal() +
...
leaflet(sdr_data_world_joined) %>% 
  ...
  addPolygons(stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, color = ~colorQuantile("inferno", goal_2_score)(goal_2_score), label = mytext)
```
