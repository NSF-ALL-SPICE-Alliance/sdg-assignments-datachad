...
```{r}
ggplot(data = sdr_data, aes(x = goal_7_score, y = goal_13_score, color = goal_5_score)) + geom_point()
```
In this chart it looks like countries with low goal_7_scores have high goal_13_scores.  Also, countries with low goal_7_scores have high goal_5_scores.
...
```{r}
mena_sdr_data <- sdr_data %>% filter(regions_used_for_the_sdr == "MENA")
```


```{r}
ggplot(mena_sdr_data, aes(x = goal_8_score) + geom_bar()
```
...
```{r}
ggplot(sdr_data, aes(x = goal_4_score, fill=regions_used_for_the_sdr)) + geom_histogram(bins = 27, color="black")
```
...
```
```{r}
ggplot(sdr_data, aes(x = goal_3_score)) +
  geom_histogram()
```
...
```{r}
ggplot(sdr_data, aes(x = goal_3_score, fill=regions_used_for_the_sdr)) + 
  geom_histogram(bins=33, color="black")
```
